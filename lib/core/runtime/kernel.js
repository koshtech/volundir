...
export class Kernel extends Base {
  ...
  async boot() {
    ...
    this.log('info', '[:Volundir][boot] Starting Application.', { session_started: this.current_user } );

    await this.events.emit('kernel:booted')

    this.log('info', '[:Volundir][boot] Application Started and Ready.', { session_started: this.current_user } );

    this.logBus.ready = true;
  }

  run () {
    console.log('info', '[:Volundir][run] Starting Application.', { session_start: this.current_session } );

    (async() => this.boot())();

    console.log('info', '[:Volundir][run] Application Started and Ready.', { session_started: this.current_session } );
  }
